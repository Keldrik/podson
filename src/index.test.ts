...
// Mock the got module with error classes for instanceof checks
vi.mock('got', () => {
...
} = vi.mocked(await import('got')) as any;

const mockedGot = got as unknown as ReturnType<typeof vi.fn>;

// Helper to set up got mock to return resolved XML
function mockGotSuccess(xml: string) {
  mockedGot.mockReturnValue({ text: () => Promise.resolve(xml) });
}

// Helper to set up got mock to reject with an error
function mockGotFailure(error: Error) {
  mockedGot.mockReturnValue({ text: () => Promise.reject(error) });
}
...
describe('getPodcast', () => {
  ...
  it('should throw error on network timeout', async () => {
    const timeoutError = new MockTimeoutError('Timeout');
    mockGotFailure(timeoutError);
  ...
  it('should throw error on HTTP error status', async () => {
    const httpError = new MockHTTPError('Not Found', 404);
    mockGotFailure(httpError);
  ...
  it('should throw error on invalid URL', async () => {
    await expect(getPodcast('not-a-url')).rejects.toThrow(
      'Invalid podcast feed URL: not-a-url'
    );
  });
  ...
  it.each([
    ...
    { input: 'ja', expected: 'ja-jp', description: 'non-western language' },
  ...
  it.each([
    ...
    async ({ statusCode }) => {
      const httpError = new MockHTTPError('HTTP Error', statusCode);
  ...
  it('should handle non-XML content gracefully', async () => {
    mockGotSuccess('This is not XML');
